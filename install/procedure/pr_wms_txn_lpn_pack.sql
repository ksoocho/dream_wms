CREATE DEFINER=`root`@`localhost` PROCEDURE `pr_wms_txn_lpn_pack`(
     IN p_org_id            INT
...
-- **********************************************
-- Procedure Name : pr_wms_lpn_pack
-- Description    : LPN Pack ( No Serial Control )
...
IF v_returnCode = 'S' THEN
    ...
    CALL pr_wms_process_lpn_txn(
			...
			, 87 -- p_txn_type_id - Container Pack 
			, p_org_id   
			, p_subinv_code  
			, p_locator_id
			, p_item_id  
